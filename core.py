...
def setup():
    ...
    e = expectation(Trusts[0,0,:])

    integral = integ.trapz(Trusts[1,1,:],dx = 1/TRUST_RESOLUTION)
...
def expectation(pdf):
    ...
    print(e)
    print('Integral', integ.trapz(pdf,rho))
    return e

def update_credence(c, msgs, exps):
    """Calculates the updated credence for an agent given new source messages

    Takes old credence @cre, all new messages @msgs and the trust functions of
    that source. Calculation according to Angere(ms) p.21 (NOT p.22).
...
def step(t):
    """This is what happens every time step.
...
for i in range(0,50):
    plt.subplot(2,1,1)
    plt.plot(rho,Trusts[0,0,:])
    if i%1 == 0:
        plt.annotate(str(i),
             xy=(rho[len(rho)-1], Trusts[0,0,len(rho) - 1]),
             xycoords='data',
